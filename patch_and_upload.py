...
def apply_patches(html):
    ...
    patches.append((
        ...
        "                const _statsBlock=state.archiveRecs.length>0?`<div class=\"crate p-4\"><h3 class=\"stencil-shadow mb-3\" style=\"color:var(--yellow)\">СТАТИСТИКА</h3><div class=\"flex flex-wrap gap-4\"><div class=\"space-y-3\" style=\"flex:2 1 220px\">${_sec('ДРОНИ',_drnRows)}${_sec('БОЄПРИПАСИ',_ammoRows)}${_sec('ПВР',_pvrRows)}</div><div style=\"flex:3 1 280px\">${_sec('РЕЗУЛЬТАТИ',_resRows)}</div></div></div>`:'';\n"
        "                _arContent=`<div style=\"display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;padding:0 8px 4px\"><h1 class=\"stencil-shadow text-3xl\" style=\"color:var(--yellow)\">${esc(state.selArchive)}</h1>${_arHeader}</div>${_statsBlock}<h2 class=\"stencil-shadow text-xl px-2\" style=\"color:var(--yellow)\">ВИЛЬОТИ</h2>${_arGroups||'<div class=\"crate p-4\"><p class=\"stencil text-center py-2\" style=\"color:var(--text-dim)\">Немає записів</p></div>'}`;\n"
        "            }\n"
        "        }\n"
        "        h+=`<div class=\"p-4 space-y-4 zvity-wrap\">\n"
