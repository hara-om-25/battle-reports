...
def apply_patches(html):
    ...
    patches.append((
        ...
        "                        const tgts=r.target?r.target.split(', '):[];\n"
        "                        const ress=r.result?r.result.split(', '):[];\n"
        "                        const parts=Array.from({length:Math.max(tgts.length,ress.length)},(_,j)=>{const tn=esc(tgts[j]||'');const rv=esc(ress[j]||'');return tn?tn+_dash+`<span style=\"color:var(--khaki)\">${rv}</span>`:rv;}).filter(Boolean);\n"
        ...
        "                _arContent=`<div style=\"display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;padding:0 8px 4px\"><h1 class=\"stencil-shadow text-3xl\" style=\"color:var(--yellow)\">${esc(state.selArchive)}</h1>${_arHeader}</div>${_arGroups||'<div class=\"crate p-4\"><p class=\"stencil text-center py-2\" style=\"color:var(--text-dim)\">Немає записів</p></div>'}`;\n"
        "            }\n"
        "        }\n"
        "        h+=`<div class=\"p-4 space-y-4 zvity-wrap\">\n"
