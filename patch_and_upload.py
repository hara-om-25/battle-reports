...
def apply_patches(html):
    ...
    patches.append((
        ...
        "                const _statsBlock=state.archiveRecs.length>0?`<div class=\"crate p-4\"><div class=\"grid grid-cols-1 gap-4 md:grid-cols-4\">${_sec('ДРОНИ',_drnRows)}${_sec('БОЄПРИПАСИ',_ammoRows)}${_sec('ПВР',_pvrRows)}${_sec('РЕЗУЛЬТАТИ',_resRows)}</div></div>`:'';\n"
