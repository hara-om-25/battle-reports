...
def apply_patches(html):
    ...
    patches.append((
        ...
        "                const _row=(label,val)=>`<div class=\"stencil\" style=\"display:flex;justify-content:space-between;padding:2px 0\"><span style=\"color:var(--text)\">${label}</span><span style=\"color:var(--yellow)\">${val}</span></div>`;\n"
        "                const _sec=(title,rows)=>rows?`<div><p class=\"stencil\" style=\"color:var(--khaki);font-size:0.85em;letter-spacing:0.08em;margin-bottom:4px\">${title}</p>${rows}</div>`:'';\n"
        "                const _drnRows=Object.entries(_drnCnt).sort((a,b)=>b[1]-a[1]).map(([d,c])=>_row(esc(d),c)).join('');\n"
        "                const _ammoRows=Object.entries(_ammoCnt).sort((a,b)=>b[1]-a[1]).map(([a,c])=>_row(esc(a),c)).join('');\n"
        ...
        "                const _resRows=Object.entries(_resCnt).sort((a,b)=>b[1]-a[1]).map(([r,c])=>_row(esc(r),c)).join('');\n"
        "                const _statsBlock=state.archiveRecs.length>0?`<div class=\"crate p-4\"><div class=\"space-y-3\">${_sec('ДРОНИ',_drnRows)}${_sec('БОЄПРИПАСИ',_ammoRows)}${_sec('ПВР',_pvrRows)}${_sec('РЕЗУЛЬТАТИ',_resRows)}</div></div>`:'';\n"
        "                _arContent=`<div style=\"display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;padding:0 8px 4px\"><h1 class=\"stencil-shadow text-3xl\" style=\"color:var(--yellow)\">${esc(state.selArchive)}</h1>${_arHeader}</div>${_statsBlock}${_arGroups||'<div class=\"crate p-4\"><p class=\"stencil text-center py-2\" style=\"color:var(--text-dim)\">Немає записів</p></div>'}`;\n"
        "            }\n"
        "        }\n"
        "        h+=`<div class=\"p-4 space-y-4 zvity-wrap\">\n"
